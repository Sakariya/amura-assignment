...
class RepositoriesController < ApplicationController
  ...
  def index
    @repositories = Github.repos.list user: @current_user.name, type: :all
  end
  ...
  def show
    ...
    repo_response = repo_conn.get(
      params[:id], access_token: @current_user.token
    )
